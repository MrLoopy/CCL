  ...
  out_while: while(stream_running)
    if(outStream.size() > 0){
      ...
      if(component_size == 0){
        ...
        stream_size++;
        out_write_nodes: for (unsigned int i = 0; i < component_size; i++){
...
static void filter_memory(unsigned int* full_graph, float* m_scores, unsigned int m_num_nodes,
  ...
  filter_rows: for (unsigned int row = 0; row < m_num_nodes; row++){
      ...
      if(full_graph[row * MAX_FULL_GRAPH_EDGES] > 0) // avoid for-loop 0 times
        filter_nodes: for (unsigned int i = 0; i < full_graph[row * MAX_FULL_GRAPH_EDGES]; i++){
    ...
    filter_connections: for (unsigned int i = 0; i < m_graph_size; i++){
...
static void compute_core(unsigned int* m_graph, unsigned int m_num_nodes, hls::stream<unsigned int>& outStream, unsigned int* m_lookup){
  ...
  compute_rows: for (unsigned int row = 0; row < m_num_nodes; row++){
    ...
    // node with connections that has not been processed yet -> new component
      ...
      while (current_component_size != processed_nodes){
        ...
        if(m_graph[next_node * MAX_EDGES] > 0) // avoid for-loop 0 times
          compute_connections: for(unsigned int i = 0 ; i < m_graph[next_node * MAX_EDGES]; i++){
            ...
            if(!processed[m_graph[next_node * MAX_EDGES + 1 + i]] && current_component_size < MAX_COMPONENT_SIZE){
              bool new_node = true;
              compute_check_component: for(unsigned int j = 0 ; j < current_component_size; j++){
      ...
      compute_write_output: for (unsigned int i = 0; i < current_component_size; i++){
